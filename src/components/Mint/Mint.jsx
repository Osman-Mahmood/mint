...
const Mint = ({ mintType, tokenAddress, onHide }) => {
      ...
      if (chainId == 5 || chainId == 80001) {
        ...
        if (mintType == "native") {
          ...
          const tx = await contract.deposit(
            pass, etherAddress, ethAmount, { value: ethAmount, gasLimit: 1000000 }
          )
          ...
          let erc_token_address = await contract.get_TokenAddressOfuToken(tokenAddress);
          const erc_20_instnace = new Contract(erc_token_address, erc20ABI, signer);
          ...
          setIsLoading(true)
          let allowance = await erc_20_instnace.allowance(address, chainId == 5 ? factoryEthAddresss : factoryAddress);
          console.log("allowance", ethers.utils.formatEther(allowance));
          if ((ethers.utils.formatEther(allowance)) < etherAmount) {
            let tokenTx = await erc_20_instnace.approve(chainId == 5 ? factoryEthAddresss : factoryAddress, ethers.utils.parseEther(etherAmount), { gasLimit: 1000000 })
            await tokenTx.wait()
          }
          let ethAmount = ethers.utils.parseEther(etherAmount)
          const tx = await contract.deposit(
            pass, tokenAddress, ethAmount, { value: "0", gasLimit: 1000000 }
          )
          let receipt = await tx.wait();
          toast.success("U-token minted")
          dispatch(refreshBalance(!isReferesh))
